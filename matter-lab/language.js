(() => {
  ...
  function localize() {
    ...
    document.querySelectorAll('.family-filters button').forEach(el=>{ const map={'Все':t.all,'Обычная':t.ordinary,'Плотная':t.dense,'Мезоны':t.mesons,'Странная':t.strange,'All':t.all,'Ordinary':t.ordinary,'Dense':t.dense,'Mesons':t.mesons,'Strange':t.strange}; setText(el,map[el.textContent.trim()]||el.textContent); });
